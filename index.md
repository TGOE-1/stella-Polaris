...
/*这是最终成果图，主要元器件包括：STC89C52RC单片机，LCD显示屏，磁传感器模块，晶振，USB-TTL转换头*/


![s1](./picture/show2.jpg)  
/*正上方表示正北，红点表示以正北为参照时，目前指南针的朝向*/
...
注意：我们所用的芯片型号为QMC5883L，由于芯片停产，**可能大家买到的标注为QMC5883L芯片实际上是QMC5883P芯片**，故这里提供两款芯片的程序并用P与L区分。磁传感器模组中的磁传感芯片（模块上最大的黑色方块）会写芯片型号，可以用放大镜查看，L芯片以HA开头，P芯片以HP开头。所用程序如下：
...
它将磁传感器采集并经由单片机处理后的地磁数据，​转换为直观、可视化的方向信息
...
单片机通过面包板与其余元器件相连接，此处介绍单片机引脚，以下是单片机的引脚示意图，用于后续单片机与其他元器件引脚连接时对照。左图是示意图，右图为实物图，注意**半圆缺口方向向上**。
...
TXD引脚负责输出数据，RXD引脚负责输入数据，故应将不同元器件的TXD与RXD相连。注意：该转换头有三个电源输出引脚，我们需要**使用5v引脚**。
...
(2) 找到stc界面中的单片机型号一栏，在列表中选择STC89C52RC/LE52RC（选择自己单片机的型号，此处仅为展示）。此时将单片机上的转换头插入电脑USB接口，软件会自动选择串口号，**如果未识别到串口号，说明你的电脑缺少ch340n的驱动**。
...
(5) 左键单击下载。注意，**单片机需要冷启动，将转换头的正极电源线从面包板上拔下再插入，然后程序才会开始烧录**。
...
#### (2)磁传感器安置与检测
我们仅用到磁传感器上的VCC（电源），GND（地线），SCL（串行时钟线），SDA（串行数据线）这四个引脚。以下是引脚连接表与示例图（表中展示的同一行的两个引脚应该相连）：
...
![s4](./picture/step5.png)![s5](./picture/step4.png)
